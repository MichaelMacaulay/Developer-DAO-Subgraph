...
export function createSweep20Event(_token: Address, _to: Address): Sweep20 {
  let sweep20Event = changetype<Sweep20>(newMockEvent())

  sweep20Event.parameters = new Array()

  sweep20Event.parameters.push(
...
export function createSweep721Event(
  _token: Address,
  _to: Address,
  _tokenID: BigInt
): Sweep721 {
  let sweep721Event = changetype<Sweep721>(newMockEvent())

  sweep721Event.parameters = new Array()

  sweep721Event.parameters.push(
    new ethereum.EventParam("_token", ethereum.Value.fromAddress(_token))
  )
  sweep721Event.parameters.push(
    new ethereum.EventParam("_to", ethereum.Value.fromAddress(_to))
